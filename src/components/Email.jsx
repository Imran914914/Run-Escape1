...
const Email = ({toggle,emailValaue}) => {
  ...
  return (
    <div className='w-full h-full flex justify-center'>
      <form onSubmit={onSubmit} className='bg-[#0f1722] w-fit rounded-md p-10'>
        <div className='w-ful h-10 flex justify-center flex-col gap-1'>
            <p className='text-white flex gap-1 justify-center text-sm'>New Here? <a href="/" className='text-blue-500'>Create an account</a></p>
        ...
        <div className='flex justify-center mt-5 w-full flex-col'>
            <p className='text-white text-3xl flex justify-start w-full'>Log into your Jagex account</p>
            <p className='text-white flex justify-center mt-6'>Log in using your email address.</p>
        </div>
        ...
        <div className="relative mt-5">
            <input value={emailValaue.email} onChange={(e)=>{emailValaue.setEmail(e.target.value)}} type="text" id="floating_outlined" className="block px-2.5 pb-2.5 pt-4 w-full text-sm text-gray-900 bg-transparent rounded-md border-1 appearance-none dark:text-white dark:border-gray-600 border-y border-x border-slate-500 focus:border-0 hover:border-slate-500 dark:focus:border-blue-500 focus:outline-dashed outline-white outline-offset-4 focus:ring-1 focus:border-blue-600 peer" placeholder=" " />
            <label htmlFor="floating_outlined" className="absolute text-sm text-[#9b9ba2] dark:text-gray-400 duration-200 transform -translate-y-4 scale-75 top-5 z-10 origin-[0] bg-white dark:bg-gray-900 px-2 peer-focus:px-2 peer-focus:text-blue-600 peer-focus:dark:text-[#9b9ba2] peer-placeholder-shown:scale-100 peer-placeholder-shown:-translate-y-1/2 peer-placeholder-shown:top-1/2 peer-focus:top-2 peer-focus:scale-75 peer-focus:-translate-y-2 rtl:peer-focus:translate-x-1/4 rtl:peer-focus:left-auto start-1">Email</label>
        </div>
        <button onClick={()=>{toggle.setTogglePass(!toggle.togglePass)}} className='bg-[#0c8ae6] w-full h-12 tsxt-sm rounded-md mt-5'><p className='text-sm'>Continue</p></button>
      </form>
    </div>
  )
}
